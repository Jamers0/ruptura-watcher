...
export function ImportDataLocal({ onDataImported }: ImportDataLocalProps) {
  ...
  const processarArquivo = async (arquivo: File) => {
    try {
      ...
      // FORÇAR salvamento local imediato
      setTimeout(() => {
        console.log('⚡ SALVAMENTO FORÇADO APÓS PROCESSAMENTO...');
        ...
        if (sucesso) {
          ...
          toast({
            title: "⚠️ Aviso de Salvamento",
            description: "Houve um problema no salvamento automático",
            variant: "destructive",
          });
        }
      }, 500);
  ...
  const processarTextoColado = () => {
    ...
    try {
      ...
      // FORÇAR salvamento local imediato
      setTimeout(() => {
        console.log('⚡ SALVAMENTO FORÇADO APÓS COLAR DADOS...');
        const todosDados = [...dadosOriginais, ...dadosProcessados];
        const sucesso = LocalStorageManager.forcarSalvamento(todosDados);
        if (sucesso) {
          toast({
      ...
      <AlertDialog open={showClearDialog} onOpenChange={setShowClearDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                Esta ação irá remover permanentemente todos os dados armazenados localmente.
              </p>
